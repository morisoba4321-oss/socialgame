...
async function fetchNaturalExampleUncached(word, opts = {}) {
  ...
  // 3) Free, human-written corpora: Tatoeba + Free Dictionary.
  // These are real sentences written and vetted by people (no API key, no
  // cost), so they are tried before any locally invented template. This is
  // the main source of natural-sounding examples in the fully free build.
  ...
  // 4) Optional AI generation through the secure serverless endpoint,
  // which calls Google's free-tier Gemini API (no cost, rate-limited).
  // Only used when no ready-made human sentence was found above, and the
  // client-side timeout keeps this from ever blocking the game for long.
  ...
  // 5) Semantic local template (meaning-aware, picked from a curated pool
  // per topic/POS). Used only when no human-written or AI sentence was found.
  ...
  // 6) Generic AI-style offline template. Last resort so the game always
  // has *something* to show even for obscure words with no corpus hit and
  // no matching semantic template.
